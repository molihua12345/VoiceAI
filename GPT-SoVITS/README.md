...
安装稳定版全家桶 (适用于 CUDA 12.1 驱动，这是目前最兼容的)
pip install torch==2.3.1 torchvision==0.18.1 torchaudio==2.3.1 --index-url https://download.pytorch.org/whl/cu121
```
2. 安装系统级 FFmpeg
即使不使用 torchcodec，torchaudio 的传统后端也需要系统中有 ffmpeg。
...
```
3. 强制指定后端 (防止它再次乱找)
在启动你的 api_v2.py 之前，在终端输入以下环境变量。这会告诉 torchaudio 忽略所有新式的 codec，老老实实用系统 ffmpeg：
